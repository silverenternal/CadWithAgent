...
fn test_extract_primitives_from_simple_room() {
    ...
    println!("线段：{}, 圆：{}, 矩形：{}", 
        result.statistics.line_count,
        result.statistics.circle_count,
        result.statistics.rect_count
    );
}
...
fn test_geometric_relations_detection() {
    ...
    println!("检测到 {} 个垂直关系", result.statistics.perpendicular_count);
...
fn test_coordinate_normalization() {
    ...
    let result = extractor.extract_from_svg_string(
...
fn test_geometry_config_validation() {
    ...
    // 测试无效角度容差
    let mut invalid_config = GeometryConfig::default();
    invalid_config.angle_tolerance = -0.01;
    assert!(invalid_config.validate().is_err());
    
    // 测试 validate_or_fix 自动修正
    let mut config_to_fix = GeometryConfig::default();
...
fn test_performance_with_many_primitives() {
    ...
    assert!(total_relations > 0, "应该检测到至少一个几何关系，实际检测到{}个", total_relations);
}
