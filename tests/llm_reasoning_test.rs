...
use cadagent::llm_reasoning::{
    LlmReasoningEngine, LlmReasoningRequest, ReasoningTask,
};
use serde_json::json;
...
fn test_count_rooms_task() {
    ...
    // 验证推理完成
    assert_eq!(response.chain_of_thought.state, cadagent::llm_reasoning::ReasoningState::Completed);
...
fn test_tool_usage_tracking() {
    ...
    assert!(response.tools_used.contains(&"analysis_execute".to_string()));
}
