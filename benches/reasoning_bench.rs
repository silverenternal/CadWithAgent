...
fn create_dense_grid(size: usize) -> Vec<Primitive> {
    ...
    for i in 0..=size {
        let x = i as f64 * 10.0;
        primitives.push(Primitive::Line(Line::from_coords([x, 0.0], [x, size as f64 * 10.0])));
    }
    
    // 水平线
    for i in 0..=size {
        let y = i as f64 * 10.0;
        primitives.push(Primitive::Line(Line::from_coords([0.0, y], [size as f64 * 10.0, y])));
    }
...
fn create_room_layout() -> Vec<Primitive> {
    ...
    // 内墙（房间分隔）
    primitives.push(Primitive::Line(Line::from_coords([200.0, 0.0], [200.0, 250.0])));
    primitives.push(Primitive::Line(Line::from_coords([0.0, 250.0], [500.0, 250.0])));
    primitives.push(Primitive::Line(Line::from_coords([300.0, 250.0], [300.0, 400.0])));
    
    // 门窗（用小线段表示）
    primitives.push(Primitive::Line(Line::from_coords([100.0, 0.0], [180.0, 0.0]))); // 门
    primitives.push(Primitive::Line(Line::from_coords([350.0, 0.0], [450.0, 0.0]))); // 窗
