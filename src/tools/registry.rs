    ...
    fn register_measurement_tools(&mut self) {
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                ...
                let start = args["start"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("start 必须是数组".into()))?;
                let end = args["end"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("end 必须是数组".into()))?;

                let start_arr = [
                    start[0].as_f64().ok_or_else(|| ToolError::InvalidArgs("start[0] 必须是数字".into()))?,
                    start[1].as_f64().ok_or_else(|| ToolError::InvalidArgs("start[1] 必须是数字".into()))?,
                ];
                let end_arr = [
                    end[0].as_f64().ok_or_else(|| ToolError::InvalidArgs("end[0] 必须是数字".into()))?,
                    end[1].as_f64().ok_or_else(|| ToolError::InvalidArgs("end[1] 必须是数字".into()))?,
                ];

                Ok(Value::Number(serde_json::Number::from_f64(measurer.measure_length(start_arr, end_arr)).unwrap()))
            }),
        });
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                let measurer = GeometryMeasurer;
                let vertices = args["vertices"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("vertices 必须是数组".into()))?;

                let coords: Vec<[f64; 2]> = vertices.iter().map(|v| {
                ...
                Ok(Value::Number(serde_json::Number::from_f64(measurer.measure_area(coords)).unwrap()))
            }),
        });
    }
    ...
    fn register_transform_tools(&mut self) {
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                ...
                let dx = args["dx"].as_f64().ok_or_else(|| ToolError::InvalidArgs("dx 必须是数字".into()))?;
                let dy = args["dy"].as_f64().ok_or_else(|| ToolError::InvalidArgs("dy 必须是数字".into()))?;
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                ...
                let primitives = args["primitives"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("primitives 必须是数组".into()))?;
                let angle = args["angle"].as_f64().ok_or_else(|| ToolError::InvalidArgs("angle 必须是数字".into()))?;
                let center = args["center"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("center 必须是数组".into()))?;

                let center_arr = [
                    center[0].as_f64().ok_or_else(|| ToolError::InvalidArgs("center[0] 必须是数字".into()))?,
                    center[1].as_f64().ok_or_else(|| ToolError::InvalidArgs("center[1] 必须是数字".into()))?,
                ];
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                ...
                let primitives = args["primitives"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("primitives 必须是数组".into()))?;
                let factor = args["factor"].as_f64().ok_or_else(|| ToolError::InvalidArgs("factor 必须是数字".into()))?;
                let center = args["center"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("center 必须是数组".into()))?;
        ...
        self.register(ToolDefinition {
            ...
            function: Arc::new(move |args| {
                ...
                let primitives = args["primitives"].as_array()
                    .ok_or_else(|| ToolError::InvalidArgs("primitives 必须是数组".into()))?;
                let axis = args["axis"].as_str().ok_or_else(|| ToolError::InvalidArgs("axis 必须是字符串".into()))?;
