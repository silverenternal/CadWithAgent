...
<svg width="800" height="600" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="575" y="350" font-size="20">门</text>
    ...
    // 保存 SVG 到临时文件
    let svg_path = "/tmp/floor_plan.svg";
    std::fs::write(svg_path, svg_content)?;
    println!("1. 创建示例 SVG 文件：{}", svg_path);
    ...
    // 步骤 2: 导出为结构化 JSON
    println!("\n3. 导出为结构化 JSON");
    let json_path = "/tmp/floor_plan.json";
    let json_result = JsonExporter::export(&svg_result.primitives, json_path)?;
    println!("   已保存：{} ({} 个图元)", json_result.path, json_result.entity_count);
    ...
    // 保存 CoT 数据
    let cot_json_path = "/tmp/floor_plan_cot.json";
    ...
    println!("   CoT 数据已保存：{}", cot_json_path);
    ...
    // 保存 QA 数据
    let qa_json_path = "/tmp/floor_plan_qa.json";
    let _qa_export_data: Vec<_> = qa_pairs.iter().map(|qa| {
        serde_json::json!({
    ...
    // 步骤 6: 导出 DXF
    println!("\n7. 导出 DXF 文件");
    let dxf_path = "/tmp/floor_plan.dxf";
    let dxf_result = DxfExporter::export(&svg_result.primitives, dxf_path)?;
    println!("   已保存：{} ({} 个图元)", dxf_result.path, dxf_result.entity_count);

    println!("\n=== 管线完成 ===");
    println!("\n生成的文件:");
    println!("  - SVG: {}", svg_path);
    println!("  - JSON: {}", json_path);
    println!("  - CoT JSON: {}", cot_json_path);
    println!("  - QA JSON: {}", qa_json_path);
    println!("  - DXF: {}", dxf_path);
